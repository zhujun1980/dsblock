...
struct FileMapping {
    ...
    FileMapping(void* _attachAddr, _Tp* _object, size_t _filesize, time_t _mtime, time_t _atime, const std::string _filepath) : attachAddr(_attachAddr), object(_object), filesize(_filesize), mtime(_mtime), atime(_atime), filepath(_filepath) {
    }
    ...
    ~FileMapping() {
        ...
        if (!MUnMap(attachAddr, filesize)) {
            DoLog(Logger::WARNING, "Unmap file %s(addr=%p, mtime=%ld, size=%d bytes) failed (%s:%d) %s[%X]",
                  filepath.c_str(), attachAddr, mtime, filesize,
                  __FILE__, __LINE__, strerror(errno), std::this_thread::get_id());
        }
        else {
            DoLog(Logger::INFO, "Unmap file %s(addr=%p, mtime=%ld, size=%d bytes) success (%s:%d)[%X]",
                  filepath.c_str(), attachAddr, mtime, filesize,
                  __FILE__, __LINE__, std::this_thread::get_id());
        }
        attachAddr = NULL;
    }
...
FileMapping<_Tp>* FileMappingCreate(const std::string& filepath) {
    ...
    if (!mapping) {
        delete obj;
        if (!MUnMap(addr, filesize)) {
            DoLog(Logger::WARNING, "unmap file failed (%s:%d) %s", __FILE__, __LINE__, strerror(errno));
        }
        return NULL;
    }
    return mapping;
