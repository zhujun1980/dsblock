...
TriSearchTries::TriSearchTries(TstNode *data, size_t frees, size_t count) :
    data_(data), free_(frees), count_(count), data_loc_(SHM_MEM) {
}
    ...
    std::vector<std::string>& words, bool with_prefix) const
{
    ...
    Traverse(pf, data_[idx].eqkid, std::bind(&PushCharToArray,
        data_, std::ref(words), _1, _2));
    return;
}
